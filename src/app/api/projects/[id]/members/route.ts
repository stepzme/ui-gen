...
export async function GET(_: Request, { params }: Params) {
  const members = Array.from(db.projectMembers.get(params.id)?.entries() || []).map(([userId, role]) => ({ userId, role }));
...
export async function POST(request: Request, { params }: Params) {
  ...
  const map = db.projectMembers.get(params.id) || new Map();
  map.set(json.userId, json.role);
  db.projectMembers.set(params.id, map);
