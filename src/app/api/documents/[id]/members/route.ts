...
export async function GET(_: Request, { params }: Params) {
  ...
  const role = await getDocumentRole(id, getSessionUserId(session));
  if (!role) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  const items = await data.listDocumentMembers(id);
...
export async function POST(request: Request, { params }: Params) {
  ...
  const { id } = await params;
  const currentRole = await getDocumentRole(id, getSessionUserId(session));
  ...
  await data.addDocumentMember(id, json.userId, json.role);
